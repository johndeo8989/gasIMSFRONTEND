...
const AddVendor = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.post("http://localhost:8080/vendor/add", data);
            handleSuccess("Vendor added!");
    ...
    return (
        <div className="p-6 lg:p-4 w-85 sm:w-[20rem] md:w-[22rem] lg:w-[22rem] xl:w-[20rem] 2xl:w-[28rem] bg-white lg:mt-9  rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-7">
                <h2 style={{ color: "#00006B" }} className="text-xl sm:text-2xl lg:text-xl font-bold">
                    Add Vendor
                </h2>
                <button
                    style={{ color: "#00006B" }}
            ...
            <form className="space-y-9 lg:space-y-6 text-zinc-400" onSubmit={handleSubmit}>
                ...
                {[
                    ...
                    <div key={name} className="relative">
                        <label className="absolute -top-4 pb-1 bg-white left-6 block text-sm">
                            {label}:
                        </label>
                        <input
                            type={type}
                            ...
                            required
                            className="mt-1 block w-full p-2 lg:p-3 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 text-sm sm:text-base"
                        />
                    </div>
                ))}
                ...
                <button
                    type="submit"
                    className="w-full p-2 lg:p-4 bg-[#00006B] hover:bg-[#10136b] text-white font-semibold rounded-md transition duration-300 ease-in-out"
