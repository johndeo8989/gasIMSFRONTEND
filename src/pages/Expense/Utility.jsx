...
import StationaryForm from "../../components/Ui/form/Expense/StationaryForm";
import UtilityForm from "../../components/Ui/form/Expense/UtilityForm";
...
const Utility = () => {
    ...
    const fetchData = (async () => {
        try {
            const response = await axios.get('http://localhost:8080/expenses/utility');
        ...
        <div className='block sm:flex relative '>
            {isFormOpen &&
                <div className={`absolute form-container bg-red-300 w-full h-screen`}>
                    {isEdit ? <UpdateExpense setIsEdit={setIsEdit} type={'Utility'} initialData={initialData} fetchData={fetchData} /> : <UtilityForm fetchData={fetchData} />}
                </div>
            }
            <div className='sm:flex-1'>
                <ExpenseLayout data={data} fetchData={fetchData} handleSearchChange={handleSearchChange} title={'Utility'}>
                    <div >
                                ...
                                <tbody>
                                    {currentviewData.map((bill, index) => (
                                        <tr className='relative' key={bill.id}>
                                            ...
                                            <td className={styles.td}>{bill.additionalInfo?.billName}</td>
                                            <td className={styles.td}>{bill.additionalInfo?.billNo}</td>
