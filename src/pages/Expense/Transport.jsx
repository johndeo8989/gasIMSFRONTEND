...
import TransportForm from "../../components/Ui/form/Expense/TransportForm";
...
const Transport = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8080/expenses/transport');
        ...
        if (e.target.value) {
            setViewData(
                [...viewData].reverse().filter((data) =>
                    ...
                    (data.longNarration?.toLowerCase().includes(e.target.value.toLowerCase())) ||
        ...
        <div className='block sm:flex relative '>
            {isFormOpen &&
                <div className={`absolute form-container bg-red-300 w-full h-screen`}>
                    {isEdit ? <UpdateExpense setIsEdit={setIsEdit} type={'Transport'} initialData={initialData} fetchData={fetchData} /> : <TransportForm fetchData={fetchData} />}

                </div>
            }
            <div className='sm:flex-1'>
                <ExpenseLayout data={data} fetchData={fetchData} handleSearchChange={handleSearchChange} title={'Transportation'}>
                    <div>
                            ...
                            <table className={styles.table}>
                                <thead>
                                    <tr className={styles.tr}>
                                        ...
                                        <th className={styles.th}>Total Person</th>
                                ...
                                <tbody>
                                    {currentviewData.map((bill, index) => (
                                        <tr className='relative' key={bill.id}>
                                            ...
                                            <td className={styles.td}>{bill.additionalInfo?.totalPerson}</td>
