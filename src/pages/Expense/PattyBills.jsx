...
import PattyBillsForm from "../../components/Ui/form/Expense/PattyBillsForm";
import axios from "axios";
import UpdateExpense from "../../components/Ui/form/Expense/UpdateExpense";
import { handleError } from "../../utils/utils";
import UpdatePetty from "../../components/Ui/form/Expense/UpdatePetty";
...
const PattyBills = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8080/expenses/petty');
        ...
        <div className='block sm:flex relative '>
            {isFormOpen &&
                <div className={`absolute form-container bg-red-300 w-full h-screen`}>
                    {isEdit ? <UpdatePetty setIsEdit={setIsEdit} type={'Patty Bills'} initialData={initialData} fetchData={fetchData} /> : <PattyBillsForm fetchData={fetchData} />}
                </div>
            }
            <div className='sm:flex-1'>
                <ExpenseLayout data={data} fetchData={fetchData} handleSearchChange={handleSearchChange} title={'Patty'}>
                    <div>
                            ...
                            <table className={styles.table}>
                                <thead>
                                    <tr className={styles.tr}>
                                        ...
                                        <th className={styles.th}>Bill Name</th>
                                        <th className={styles.th}>Bill No</th>
                                ...
                                <tbody>
                                    {currentviewData.map((bill, index) => {
                                        let info = {};
                                        try {
                                            info = JSON.parse(bill.additionalInfo || '{}');
                                        } catch (e) {
                                            console.error('Invalid JSON in additionalInfo:', e);
                                        }

                                        const isActive = bill._id === id;

                                        return (
                                            <tr className='relative' key={bill._id}>
