...
const Stationary = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:8080/expenses/stationary');
        ...
        <div className='block sm:flex relative'>
            {isFormOpen &&
                <div className={`absolute form-container bg-red-300 w-full h-screen`}>
                    {isEdit ? <UpdateExpense setIsEdit={setIsEdit} type={'Stationary'} initialData={initialData} fetchData={fetchData} /> : <StationaryForm fetchData={fetchData} />}
                </div>
            }
            <div className='sm:flex-1'>
                <ExpenseLayout data={data} fetchData={fetchData} handleSearchChange={handleSearchChange} title={'Stationary'}>
                    <div >
                                ...
                                <tbody>
                                    {currentviewData.map((bill, index) => (
                                        <tr className='relative' key={bill._id}>
                                            ...
                                            <td className={styles.td}>{bill.additionalInfo?.item}</td>
