...
const ExpenseOverview = () => {
    ...
    const fetchProducts = async () => {
        try {
            const res = await axios.get("http://localhost:8080/profit/all", { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
            console.log(res.data)
            setBills((res.data.profit).reverse())
            setData((res.data.profit).reverse())
        ...
        if (e.target.value) {
            setBills(
                [...data].reverse().filter((data) =>
                    ...
                    (data.perUnitPrice?.toString().toLowerCase().includes(e.target.value.toLowerCase())) ||
                    (data.productName?.toLowerCase().includes(e.target.value.toLowerCase())) ||
                    (data.quantity?.toLowerCase().includes(e.target.value.toLowerCase())) ||  // Fixed: added optional chaining
                    (data.totalPrice?.toString()?.toLowerCase().includes(e.target.value.toLowerCase()))
                )
            );
        } else {
    ...
    const onSortChange = (order) => {
        if (order === 'desc')
            setBills([...data].sort(function (a, b) { return b.totalPrice - a.totalPrice }))
        else if (order === 'asc') setBills([...data].sort(function (a, b) { return a.totalPrice - b.totalPrice }))
    }
    ...
    useEffect(() => {
        settotalProfit(calculateSum(bills.map(exp => exp.totalPrice)))
        setTotalSalary(calculateSum(bills.filter(exp => exp.expenseType === 'salary').map(salary => salary.expensePrice)))
        setTotalBills(calculateSum(bills.filter(exp => exp.expenseType === 'utility').map(bills => bills.expensePrice)))
        setTotalRent(calculateSum(bills.filter(exp => exp.expenseType === 'rent').map(rent => rent.expensePrice)))
    }, [bills])
    ...
    const exportProductsToPDF = (bills) => {
        const doc = new jsPDF();
        const tableColumn = ["S.No", "Date", "Product Name", 'Quantity', "Per Price", "Total Profit",];
        const tableRows = [];
    ...
    const exportProductsToExcel = (bills) => {
        const data = bills.map((bill, index) => ({
            ...
            "Per Price": bill.perUnitPrice,
            "Total Profit": bill.totalPrice
        }));
        const worksheet = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Products");
        XLSX.writeFile(workbook, "Profit.xlsx");
    };
        ...
        <div className='block sm:flex relative mt-4'>
            {isFormOpen &&
                <div className={`absolute form-container bg-red-300 w-full h-screen`}>
                    <AddProfit fetchProducts={fetchProducts} />
                </div>
            }
            <div className='sm:pr-5 flex-1'>
                        ...
                        <div className='relative '>
                            <span onClick={toggleSort}>
                                {!showShort ?
                                    ...
                                    <RxCross2 onClick={() => setBills([...data].reverse())
                                    } />}
                ...
                <div className={styles.card_container}>
                    <div>
                        <b>₹ <span className='tracking-wider'>{formattedNumber(totalProfit)}</span></b>
                        <p>
                            <span>
                            ...
                            <table className={styles.table}>
                                <thead>
                                    <tr className={styles.tr}>
                                        ...
                                        <th className={styles.th}>Per Price</th>
                                ...
                                <tbody>
                                    {
                                        ...
                                        currentBills.reverse().map((bill, index) => (
                                            <tr className='relative' key={bill.id}>
                                                ...
                                                <td className={styles.td}>{bill?.productName?.toUpperCase()}</td>
                                                <td className={styles.td}>{bill?.quantity}</td>
                                                <td className={styles.td}>{bill?.perUnitPrice}</td>
                                                <td className={styles.td}>₹{bill?.totalPrice}</td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
